...
#./Configure android-ndk --prefix="${PREFIX}" --openssldir="${PREFIX}/share" android64-aarch64 'no-shared' || exit 1
./Configure --prefix="${PREFIX}" --openssldir="${PREFIX}/share" android64-aarch64-clang 'no-shared' || exit 1
##./Configure --prefix="/usr" --openssldir="/etc/ssl" android64-aarch64-clang 'no-shared' || exit 1
