...
./configure --prefix="${PREFIX}" --host="${TARGET}" --build="${HOST}" --without-ada --without-manpages --without-progs --without-tests --without-termlib --enable-termcap --enable-widec --disable-database --disable-home-terminfo --with-shared --without-normal --without-debug --without-shlib-version || exit 1
make || exit 1
make install || exit 1
...
rm ${PREFIX}/lib/libncurses++w.a || exit 1
